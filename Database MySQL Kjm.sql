...
CREATE TABLE data_type (
    ...
    column3 BOOLEAN, 		# 논리형 데이터 타입 (0: false, 1: true 로 결과값이 나옴)
    column4 VARCHAR(10),	# 문자열 데이터 타입 (길이를 포함하고 정해야함)
    column5 TEXT,			# 문자열 데이터 타입 (길이 제한을 적지않음)
    column6 DATE,			# 날짜  데이터 타입 
    column6 TIME, 			# 시간  데이터 타입 
    column7 DATETIME		# 날짜 및 시간 데이터 타입 
);
