...
VALUES ('email3@email.com', 'qlalfqjsgh', 'nickname3', '01011111113', '부산광역시 부산진구', '에이원프라자', true);

# 로그인 SQL
SELECT * FROM user WHERE email = 'email@email.com' AND password = 'qlalfqjsgh';
...
# 좋아용 등록 SQL
INSERT INTO favorite VALUES (4, 'email3@email.com');
...
ORDER BY B.writer_datetime DESC;
...
# 게시물 번호가 2번인 게시물을 조회 
# (게시물번호, 제목, 내용, 작성날짜 및 시간, 작성자 이메일, 작성자 닉네임, 작성자 프로필 사진 )
SELECT 
	B.board_number, 
	B.title, 
	B.contents, 
...
FROM board AS B INNER JOIN user AS U 
ON B.writer_email = U.email
WHERE B.board_number = 2; # LIKE '%2%';

# 최신 게시물 리스트 SQL 찐
# 게시물번호, 프로필이미지, 닉네임, 작성날짜, 제목, 내용, 댓글 수, 좋아요 수, 조회 수, 사진
# 필요 테이블 = board, user, favorite, comment
SELECT 
